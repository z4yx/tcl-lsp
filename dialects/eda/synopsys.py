...
def synopsys_command_specs() -> tuple[CommandSpec, ...]:
    ...
    return (
        ...
        _syn(
            ...
            "all_fanin -to sink_list ?-startpoints_only? ?-exclude_bboxes? ?-break_on_bboxes? ?-only_cells? ?-flat? ?-levels count? ?-trace_arcs arc_type? ?-continue_trace pin_types?",
        ),
        # cat2/all_fanout.2
        _syn(
            "all_fanout",
            "Returns a set of pins, ports, or cells in the fanout of the specified sources.",
            "all_fanout -clock_tree -from source_list ?-endpoints_only? ?-exclude_bboxes? ?-break_on_bboxes? ?-only_cells? ?-flat? ?-levels count? ?-trace_arcs arc_type? ?-continue_trace pin_types?",
        ),
        ...
        _syn(
            ...
            "all_inputs ?-clock clock_name? ?-edge_triggered | -level_sensitive? ?-exclude_clock_ports?",
        ),
        ...
        _syn(
            ...
            "all_outputs ?-clock clock_name? ?-edge_triggered | -level_sensitive?",
        ),
        ...
        # cat2/all_registers.2
        _syn(
            "all_registers",
            "Returns a collection of sequential cells or pins in the current design.",
            "all_registers ?-no_hierarchy? ?-clock clock_name? ?-rise_clock rise_clock_name? ?-fall_clock fall_clock_name? ?-cells? ?-data_pins? ?-clock_pins? ?-slave_clock_pins? ?-output_pins? ?-level_sensitive | -edge_triggered? ?-master_slave? ?-include_icg? ?-include_operator_register?",
        ),
        ...
        # cat2/append_to_collection.2
        _syn(
            "append_to_collection",
            "Adds objects to a collection and modifies a variable.",
            "append_to_collection ?-dict dict_var? ?-unique? var_name object_spec",
        ),
        ...
        _syn(
            ...
            "check_timing ?-overlap_tolerance minimum_distance? ?-override_defaults check_list? ?-multiple_clock? ?-sdc_runtime? ?-retain? ?-include check_list? ?-exclude check_list? ?-nosplit?",
        ),
        ...
        # cat2/create_generated_clock.2
        _syn(
            "create_generated_clock",
            "Creates a generated clock object.",
            "create_generated_clock ?-name clock_name? ?-add? source_objects -source master_pin ?-master_clock clock? ?-divide_by divide_factor | -multiply_by multiply_factor? ?-duty_cycle percent? ?-invert? ?-preinvert? ?-edges edge_list? ?-edge_shift edge_shift_list? ?-combinational? ?-comment comment_string?",
        ),
        # cat2/create_lib.2
        _syn(
            "create_lib",
        ...
        _syn(
            ...
            "define_proc_attributes proc_name ?-info info_text? ?-define_args arg_defs? ?-define_arg_groups group_defs? ?-command_group group_name? ?-return type_name? ?-hide_body? ?-hidden? ?-dont_abbrev? ?-permanent? ?-deprecated? ?-obsolete?",
        ),
        # cat2/define_qor_data_panel.2
        _syn(
            "define_qor_data_panel",
        ...
        _syn(
            "get_cells",
            "Creates a collection of cells from the current design, relative to the current instance. Creates a collection of cells that match the specified criteria.",
            "get_cells ?-quiet? ?-regexp | -exact? ?-nocase? ?-filter expression? ?-hierarchical? ?-rtl? ?patterns | -of_objects objects | -within region | -touching region | -intersect region | -at point? ?-all?",
        ),
        # cat2/get_clocks.2
        _syn(
            "get_clocks",
        ...
        # cat2/get_lib_cells.2
        _syn(
            "get_lib_cells",
            "Creates a collection of library cells from the libraries loaded into memory.",
            "get_lib_cells ?-quiet? ?-regexp | -exact? ?-nocase? ?-filter expression? ?-scenarios scenario_names? patterns | -of_objects objects",
        ),
        # cat2/get_lib_pins.2
        _syn(
            "get_lib_pins",
            "Creates a collection of library cell pins from libraries loaded into memory.",
            "get_lib_pins ?-quiet? ?-regexp | -exact? ?-nocase? ?-filter expression? patterns | -of_objects objects",
        ),
        # cat2/get_libs.2
        _syn(
            "get_libs",
            "Creates a collection of libraries loaded into memory.",
            "get_libs ?-quiet? ?-regexp | -exact? ?-nocase? ?-filter expression? ?-scenarios scenario_names? ?patterns | -of_objects objects?",
        ),
        # cat2/get_license.2
        _syn(
            "get_license",
        ...
        _syn(
            ...
            "get_pins ?-quiet? ?-regexp | -exact? ?-nocase? ?-filter expression? ?-hierarchical? ?patterns | -of_objects objects ?-leaf??",
        ),
        ...
        _syn(
            ...
            "get_ports ?-quiet? ?-regexp | -exact? ?-nocase? ?-filter expression? ?-hierarchical? ?patterns | -of_objects objects?",
        ),
        ...
        _syn(
            ...
            "report_area ?-nosplit? ?-physical? ?-hierarchy? ?-designware? ?-individual_site_def_util?",
        ),
        ...
        _syn(
            ...
            "report_clock ?-attributes? ?-skew? ?-nosplit? ?-groups? ?-scenarios scenario_list? ?clock_names? ?-exclusivity?",
        ),
        ...
        # cat2/report_clock_timing.2
        _syn(
            "report_clock_timing",
            "Reports the timing attributes of clock networks.",
            "report_clock_timing -type report_type ?-clock clock_list? ?-from_clock from_clock_list? ?-to_clock to_clock_list? ?-to to_list? ?-from from_list? ?-setup? | ?-hold? ?-launch? | ?-capture? ?-rise? | ?-fall? ?-min? | ?-max? ?-nworst worst_entries? ?-greater_than lower_limit? ?-lesser_than upper_limit? ?-slack_lesser_than slack_upper_limit? ?-include_uncertainty_in_skew? ?-verbose? ?-show_clocks? ?-nosplit? ?-significant_digits digits? ?-nets? ?-capacitance? ?-attributes? ?-physical? ?-filter_false_paths? ?-variation? ?-scenarios scenario_list? ?-max? ?-fall? ?-capture? ?-hold?",
        ),
        ...
        _syn(
            ...
            "report_power ?-net? ?-cell? ?-only cell_or_net_list? ?-hierarchy? ?-levels level_value? ?-verbose? ?-flat? ?-exclude_boundary_nets? ?-include_input_nets? ?-analysis_effort low | medium | high? ?-nworst number? ?-sort_mode mode? ?-histogram ?-exclude_leq le_val | -exclude_geq ge_val?? ?-nosplit? ?-scenarios scenario_list? ?-groups group_list?",
        ),
        ...
        # cat2/report_timing.2
        _syn(
            "report_timing",
            "Displays timing information about a design.",
            "report_timing ?-to to_list | -rise_to rise_to_list | -fall_to fall_to_list? ?-from from_list | -rise_from rise_from_list | -fall_from fall_from_list? ?-through through_list? ?-rise_through rise_through_list? ?-fall_through fall_through_list? ?-exclude exclude_list | -rise_exclude rise_exclude_list | -fall_exclude fall_exclude_list? ?-path_type short | full | full_clock | full_clock_expanded | only | end? ?-delay_type min | min_rise | min_fall | max | max_rise | max_fall? ?-nworst paths_per_endpoint? ?-max_paths max_path_count? ?-input_pins? ?-nets? ?-transition_time? ?-crosstalk_delta? ?-capacitance? ?-effective_capacitance? ?-attributes? ?-physical? ?-slack_greater_than greater_slack_limit? ?-slack_lesser_than lesser_slack_limit? ?-lesser_path max_path_delay? ?-greater_path min_path_delay? ?-loops? ?-enable_preset_clear_arcs? ?-significant_digits digits? ?-nosplit? ?-sort_by group | slack? ?-group group_name? ?-trace_latch_borrow? ?-derate? ?-normalized_slack? ?-scenarios scenario_list? ?-temperature? ?-voltage? ?-unique_pins? ?-start_end_pair? ?-variation? ?-ignore_infeasible_paths?",
        ),
        ...
        _syn(
            ...
            "set_clock_groups -physically_exclusive | -logically_exclusive | -asynchronous ?-allow_paths? ?-name clock_group_name? -group clock_list ?-comment comment_string?",
        ),
        ...
        # cat2/set_disable_timing.2
        _syn(
            "set_disable_timing",
            "Disables timing arcs in the current design.",
            "set_disable_timing object_list ?-from from_pin_name -to to_pin_name? ?-restore?",
        ),
        ...
        _syn(
            "set_dont_use",
            "Sets the dont_use attribute on library cells, modules, and implementations. Library objects with the dont_use attribute are excluded from the target library during optimization. In addition, if you optimize a design that contains cell instances that are mapped to standard cells in the target library that have the dont_use attribute, those cell instances are remapped to equivalent standard cells from the target library that do not have the dont_use attribute.",
            "set_dont_use ?-power? object_list",
        ),
        ...
        # cat2/set_false_path.2
        _syn(
            "set_false_path",
            "Removes timing constraints from particular paths.",
            "set_false_path ?-rise | -fall? ?-setup | -hold? ?-from from_list | -rise_from rise_from_list | -fall_from fall_from_list? ?-through through_list? ?-rise_through rise_through_list? ?-fall_through fall_through_list? ?-to to_list | -rise_to rise_to_list | -fall_to fall_to_list? ?-reset_path? ?-comment comment_string?",
        ),
        ...
        _syn(
            ...
            "set_ideal_network object_list ?-dont_care_placement? ?-no_propagate?",
        ),
        ...
        _syn(
            ...
            "set_max_area ?-ignore_tns? area_value",
        ),
        # cat2/set_max_capacitance.2
        _syn(
            "set_max_capacitance",
            "Sets the max_capacitance attribute to a specified value on the specified clocks, ports and designs.",
            "set_max_capacitance capacitance_value ?-data_path? ?-clock_path? object_list",
        ),
        # cat2/set_max_delay.2
        _syn(
            "set_max_delay",
            "Specifies a maximum delay target for paths in the current design.",
            "set_max_delay delay_value ?-rise | -fall? ?-from from_list | -rise_from rise_from_list | -fall_from fall_from_list? ?-through through_list? ?-rise_through rise_through_list? ?-fall_through fall_through_list? ?-to to_list | -rise_to rise_to_list | -fall_to fall_to_list? ?-group_path group_name? ?-reset_path? ?-comment comment_string? ?-ignore_clock_latency?",
        ),
        # cat2/set_max_fanout.2
        _syn(
            "set_max_fanout",
        ...
        # cat2/set_min_delay.2
        _syn(
            "set_min_delay",
            "Specifies a minimum delay target for paths in the current design.",
            "set_min_delay delay_value ?-rise | -fall? ?-from from_list | -rise_from rise_from_list | -fall_from fall_from_list? ?-through through_list? ?-rise_through rise_through_list? ?-fall_through fall_through_list? ?-to to_list | -rise_to rise_to_list | -fall_to fall_to_list? ?-reset_path? ?-comment comment_string? ?-ignore_clock_latency?",
        ),
        ...
        _syn(
            ...
            "set_units ?-resistance ohm|kohm|mohm|Mohm|10ohm|100ohm? ?-capacitance f|ff|pf|nf|uf|mf|10ff|100ff? ?-time s|fs|ps|ns|us|ms|10ps|100ps? ?-power w|fw|pw|nw|uw|mw|10uw|100uw|10mw|100mw|10pw|100pw|10nw|100nw? ?-current A|fA|pA|nA|uA|mA|10uA|100uA|10mA|100mA? ?-voltage v|fv|pv|nv|uv|mv|10mv|100mv?",
        ),
        ...
        # cat2/sizeof_collection.2
        _syn(
            "sizeof_collection",
            "Returns the number of objects in a collection.",
            "sizeof_collection ?-categorize? collection1",
        ),
